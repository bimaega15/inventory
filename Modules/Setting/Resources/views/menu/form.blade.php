            ...
            <div class="row">
                <div class="col-lg-6">
                    <x-form-checkbox-vertical label="Apakah Menu ini induk ?" name="is_node" placeholder="Nama Aplikasi..."
                        checked="{{ isset($menu) ? ($menu->is_node != null ? 'checked' : '') : '' }}" labelInput="Iya" />
                </div>
                <div class="col-lg-6">
                    <x-form-checkbox-vertical label="Apakah Menu ini sebagai children ?" name="is_children"
                        placeholder="Nama Aplikasi..."
                        checked="{{ isset($menu) ? ($menu->is_children != null ? 'checked' : '') : '' }}"
                        labelInput="Iya" />
                </div>
            </div>
            <div class="col-span-12 sm:col-span-12 mb-2 {{ isset($menu) ? ($menu->is_children != null || $menu->is_children == 1 ? '' : 'd-none') : 'd-none' }}"
                id="form-menu_root_id">
                <x-form-select-vertical label="Daftar Menu" name="menu_root" :data="$array_menu" value=""
                    class="select2" />
            </div>
            <div class="col-span-12 sm:col-span-12 mb-2 {{ isset($menu) ? ($menu->is_node != null || $menu->is_node == 1 ? '' : 'd-none') : '' }}"
